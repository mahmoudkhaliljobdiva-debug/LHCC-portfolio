...
function remainingLabel(user: PlatformUser, status: EffectiveUserStatus): string { if (status === "expired") return "Expired"; if (status === "inactive") return "Inactive"; if (user.role === "student") return "No account expiration"; if (!user.expirationDate) return "Not configured"; const days = getRemainingDays(user.expirationDate); return `${days} ${days === 1 ? "day" : "days"}`; }
