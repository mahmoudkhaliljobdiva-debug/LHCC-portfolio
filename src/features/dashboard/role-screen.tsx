...
export async function RoleScreen({ role, section }: { readonly role: UserRole; readonly section: string | undefined }) {
  if (section === "portfolio" && role === "admin") return <PortfolioEditor initialContent={await getPortfolioContent()} />;
  ...
  if (section === "question-banks" && role === "teacher") return <Page title={title} subtitle="Real course and learner performance data from Supabase."><TeacherQuestionBankGrid banks={await getTeacherBankSummaries()} /></Page>;
  if (section === "analytics") return <AnalyticsPage title={title} data={data} />;
  if (section === "profile" || section === "settings") return <ProfilePage title={title} role={role} displayName={data.displayName} />;
  if (section === "questions") return <RealFeatureState title={title} message="Manage published questions from their question bank." href={role === "admin" ? "/admin/question-banks" : "/teacher/question-banks"} action="Open question banks" />;
  if (section === "exams") return <RealFeatureState title={title} message="No exams have been configured in the database yet." />;
...
function DashboardHome({ role, data }: { readonly role: UserRole; readonly data: DashboardData }) {
  return <Page title={role === "admin" ? "Platform overview" : `Welcome, ${data.displayName}`} subtitle="Live platform metrics calculated from persisted records.">
