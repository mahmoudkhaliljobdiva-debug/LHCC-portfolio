...
async function review(requestId: string, decision: "APPROVED" | "REJECTED", reason: string | null): Promise<ServerResult<null>> {
  ...
  try {
    ...
    const { error } = await db.rpc("review_bank_access", { request_id: parsed.data.id, decision, reason: parsed.data.reason });
...
export async function submitBankAnswer(questionId: string, optionId: string): Promise<ServerResult<boolean>> {
  try {
